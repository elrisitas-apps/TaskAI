...
export default function SignUpScreen() {
  ...
  const [name, setName] = useState(COMMON_STRINGS.Empty);
        ...
        <ScrollView contentContainerStyle={styles.scrollContent}>
          <View style={styles.header}>
            <Text style={[styles.title, { color: semantic.text(isDark) }]}>
              {AUTH_STRINGS.CREATE_ACCOUNT}
            </Text>
            <Text style={[styles.subtitle, { color: semantic.textSecondary(isDark) }]}>
              {AUTH_STRINGS.SIGN_UP_TO_GET_STARTED}
            </Text>
          </View>

          <View style={styles.form}>
            <Input
              label={FORM_STRINGS.NAME}
              value={name}
              onChangeText={setName}
              placeholder={FORM_STRINGS.ENTER_NAME}
            ...
            <Button
              title={AUTH_STRINGS.SIGN_UP}
              onPress={handleSignUp}
              loading={isLoading}
              style={styles.button}
            />
            <Button
              title={AUTH_STRINGS.SIGN_UP_WITH_GOOGLE}
